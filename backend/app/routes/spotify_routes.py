...
@router.get("/callback")
async def spotify_callback(request: Request, service: SpotifyService = Depends(get_spotify_service)):
    try:
...
async def create_playlist(request: PlaylistCreateRequest, service: SpotifyService = Depends(get_spotify_service)):
    try:
...
async def search_track(artist: str, track: str, service: SpotifyService = Depends(get_spotify_service)):
    try:
...
async def add_tracks_to_playlist(playlist_id: str, track_uris: list[str], service: SpotifyService = Depends(get_spotify_service)):
    try:
