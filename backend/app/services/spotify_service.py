...
class SpotifyService:
    ...
    async def get_playlist(self, playlist_id: str) -> Playlist:
        try:
            ...
            raise SpotifyServiceError(
                f"Failed to fetch playlist from Spotify for: {playlist_id}"
            ) from exception
    ...
    async def create_playlist(self, request: PlaylistCreateRequest) -> None:
        try:
            ...
            raise SpotifyServiceError(
                f"Failed to create playlist on Spotify"
            ) from exception
    ...
    async def search_track(self, artist: str, track: str) -> None:
        try:
            ...
            raise SpotifyServiceError(
                f"Failed to search tracks on Spotify"
            ) from exception
    ...
    async def add_tracks_to_playlist(
        ...
        try:
            ...
            return await self.spotify_client.add_tracks_to_playlist(
                playlist_id, track_uris
            )
        except Exception as exception:
            logger.error(f"Failed to add tracks to playlist on Spotify")
            raise SpotifyServiceError(
                f"Failed to add tracks to playlist on Spotify"
            ) from exception
